...
import { analyseImageFlow } from "../flows/analyseImageFlow";
...
describe("analyseImageFlow (handleMedia)", () => {
    ...
    it("should send the image analysis response", async () => {
        mockGetMessage.mockReturnValue("We're analyzing your image. Please wait...");

        await action(mockCtx, mockProvider);

        expect(messageService.sendMessage).toHaveBeenCalled();
        const sendMessageCalls = (messageService.sendMessage as jest.Mock).mock.calls;
  ...
  it("should return a valid analysis type", async () => {
    ...
    expect(promptBuilder.generateImageAnalysisPrompt).toHaveBeenCalledWith('Analyze this image in detail');
    expect(sendChatMessage).toHaveBeenCalled();
    expect(mockDb.query).toHaveBeenCalledWith(
      expect.stringContaining('UPDATE $linkedTask SET model_task = $model_task'),
