...
function retry() {
  ...
  if (fs$C[gracefulQueue].length === 0) return;
...
utils$c.checkPath = function checkPath(pth) {
  if (process.platform === "win32") {
    const pathHasInvalidWinCharacters = /[<>:"|?*]/.test(
      pth.replace(path$p.parse(pth).root, "")
    );
...
async function copy$5(src, dest, opts = {}) {
  ...
  const { srcStat, destStat } = await stat$9.checkPaths(
    src,
    dest,
    "copy",
    opts
...
async function onDir$3(srcStat, destStat, src, dest, opts) {
  ...
  await Promise.all(
    items.map(async (item) => {
      ...
      const { destStat } = await stat$9.checkPaths(
        srcItem,
        destItem,
        "copy",
        opts
...
function getStats$3(destStat, src, dest, opts) {
  ...
  if (srcStat.isDirectory()) return onDir$2(srcStat, destStat, src, dest, opts);
    ...
    return onFile$2(srcStat, destStat, src, dest, opts);
...
const emptyDir$1 = u$j(async function emptyDir(dir) {
  ...
  return Promise.all(
    items.map((item) => remove$4.remove(path$l.join(dir, item)))
  );
});
...
const { symlinkPaths: symlinkPaths$2, symlinkPathsSync: symlinkPathsSync$2 } =
  symlinkPaths_1$1;
const { symlinkType: symlinkType$2, symlinkTypeSync: symlinkTypeSync$2 } =
  symlinkType_1$1;
...
async function move$3(src, dest, opts = {}) {
  ...
  const { srcStat, isChangingCase = false } = await stat$7.checkPaths(
    src,
    dest,
    "move",
    opts
...
function moveSync$1(src, dest, opts) {
  ...
  const { srcStat, isChangingCase = false } = stat$6.checkPathsSync(
    src,
    dest,
    "move",
    opts
...
const emptyDirSessions = async (pathBase) =>
...
function errSerializer$1(err) {
  ...
  _err.type =
    toString$1.call(err.constructor) === "[object Function]"
          ...
          if (
            ...
            nv =
              i !== lastPathIndex
                ...
                : censor;
            if (kIsWc) {
              const rv = restoreInstr(
                node$1(redactPathCurrent, wck, depth),
                ov,
                o[originalKey]
              );
              store.push(rv);
              ...
              if (wcov[k] === nv);
                ...
                redactPathCurrent = node$1(redactPathCurrent, wck, depth);
                const rv = restoreInstr(
                  node$1(redactPathCurrent, k, depth + 1),
                  ov,
                  o[originalKey]
                );
                store.push(rv);
...
const { redactFmtSym: redactFmtSym$3, wildcardFirstSym: wildcardFirstSym$1 } =
  symbols$1;
...
function openFile(file, sonic) {
  ...
  if (sonic.sync) {
    try {
      if (sonic.mkdir)
        fs$l.mkdirSync(path$d.dirname(file), { recursive: true });
...
warning$1.create(
  warnName,
  "PINODEP008",
  "prettyPrint is deprecated, look at https://github.com/pinojs/pino-pretty for alternatives."
);

warning$1.create(
  warnName,
  "PINODEP009",
  "The use of pino.final is discouraged in Node.js v14+ and not required. It will be removed in the next major version"
);
...
function requireThreadStream() {
  ...
  const FinalizationRegistry =
    commonjsGlobal.FinalizationRegistry ||
...
function genLog$1(level, hook) {
  ...
  function LOG(o, ...n) {
    if (typeof o === "object") {
      ...
      this[writeSym$1](
        o,
        format(msg, formatParams, this[formatOptsSym$2]),
        level
      );
    } else {
...
function asJson$1(obj, msg, num, time) {
  ...
  if (this[nestedKeySym$1] && propStr) {
    ...
    return (
      data + this[nestedKeyStrSym$1] + propStr.slice(1) + "}" + msgStr + end
    );
  } else {
  ...
  return {
    ...
    chindings() {
      ...
      if (lastLogger.hasOwnProperty(parsedChindingsSym$1)) {
        ...
        chindings = JSON.parse(
          "{" + lastLogger[chindingsSym$2].substr(1) + "}"
        );
    ...
    write(chunk) {
      ...
      if (
        lastMsg &&
        formattedObj &&
        !Object.prototype.hasOwnProperty.call(formattedObj, messageKey)
      ) {
...
function assertDefaultLevelFound$1(
  defaultLevel,
  customLevels,
  useOnlyCustomLevels
) {
...
var keywords$1 = ["fast", "logger", "stream", "json"];
...
function child(bindings, options) {
  ...
  if (options.hasOwnProperty("customLevels") === true) {
    ...
    instance.levels = mappings$1(
      options.customLevels,
      instance[useOnlyCustomLevelsSym$1]
    );
...
function write(_obj, msg, num) {
  ...
  const mixinMergeStrategy =
    this[mixinMergeStrategySym$1] || defaultMixinMergeStrategy;
  let obj;
    ...
    if (!destStat) {
      ...
      fs$f.readlink(dest, (err, resolvedDest) => {
        ...
        if (
          ...
          stat$4.isSrcSubdir(resolvedDest, resolvedSrc)
        ) {
          return cb(
            new Error(
              `Cannot overwrite '${resolvedDest}' with '${resolvedSrc}'.`
...
function createSymlink$1(srcpath, dstpath, type, callback) {
  ...
  fs$6.lstat(dstpath, (err, stats) => {
    if (!err && stats.isSymbolicLink()) {
      Promise.all([fs$6.stat(srcpath), fs$6.stat(dstpath)]).then(
        ([srcStat, dstStat]) => {
...
function moveSync(src, dest, opts) {
  ...
  const { srcStat, isChangingCase = false } = stat$1.checkPathsSync(
    src,
    dest,
    "move",
    opts
 ...
 */
var enhanceError$1 = function enhanceError(
  error,
  config,
  code,
  request,
...
var defaults$3 = {
  ...
  transformResponse: [
    function transformResponse(data) {
      ...
      if (
        strictJSONParsing ||
        (forcedJSONParsing && utils$5.isString(data) && data.length)
      ) {
        try {
...
class AbstractTokenizer {
  ...
  async readToken(token, position = this.position) {
    ...
    if (len < token.len) throw new peek_readable_1$3.EndOfStreamError();
    ...
    if (normOptions.length > 0) {
      try {
        ...
        if (
          options &&
          options.mayBeLess &&
          err instanceof peek_readable_1$2.EndOfStreamError
        ) {
          return 0;
        }
        throw err;
...
const { stringToBytes, tarHeaderChecksumMatches, uint32SyncSafeToken } = util;
...
const convert = (
  ...
  {
    quality = 100,
    background = Utils_1$2.defaultBg,
    type = StickerTypes_1.StickerTypes.DEFAULT,
  }
) =>
  __awaiter$2(void 0, void 0, void 0, function* () {
    ...
    if (isAnimated && ["crop", "circle", "rouded"].includes(type)) {
      const filename = `${(0, os_1.tmpdir)()}/${Math.random().toString(
        36
      )}.webp`;
    ...
    const img = (0, sharp_1.default)(image, { animated: isAnimated }).toFormat(
      "webp"
    );
    switch (type) {
...
if (typeof window === "undefined") {
  ...
  let f = async () => {
    throw new Error(
      "Running inside a browser; filesystem support is not available"
    );
  };
  fs = {
    read: f,
...
let WebPReader$1 = class WebPReader {
  ...
  async read() {
    if (this.type == intfTypes.FILE) {
      this.fp = await IO$1.open(this.path, "r");
    }
  ...
  async commit() {
    ...
    if (this.type == intfTypes.FILE) {
      ...
      for (let i = 0, l = chunks.length; i < l; i++) {
        await IO$1.write(fp, chunks[i], 0, undefined, undefined);
      }
      await IO$1.close(fp);
    } else {
      ...
      return function (LibWebP) {
        ...
        if (ENVIRONMENT_IS_NODE) {
          if (ENVIRONMENT_IS_WORKER) {
            scriptDirectory = require$$1$1.dirname(scriptDirectory) + "/";
          } else {
        ...
        Module["_decodeRGBA"] = function () {
          return (Module["_decodeRGBA"] = Module["asm"]["t"]).apply(
            null,
            arguments
          );
        };
        Module["_decodeFree"] = function () {
          return (Module["_decodeFree"] = Module["asm"]["u"]).apply(
            null,
            arguments
          );
        };
        Module["_allocBuffer"] = function () {
          return (Module["_allocBuffer"] = Module["asm"]["v"]).apply(
            null,
            arguments
          );
        };
        ...
        Module["_destroyBuffer"] = function () {
          return (Module["_destroyBuffer"] = Module["asm"]["x"]).apply(
            null,
            arguments
          );
        };
        ...
        Module["dynCall_jiiiii"] = function () {
          return (Module["dynCall_jiiiii"] = Module["asm"]["F"]).apply(
            null,
            arguments
          );
        };
...
function requireLibwebp() {
  ...
  libwebp = class libWebP {
    ...
    async init() {
      ...
      this.api.getResult = (e) => {
        return new Uint8Array(
          new Uint8Array(Module.HEAP8.buffer, e.getResult(), e.getResultSize())
        );
      };
    ...
    encodeImage(
      ...
      width,
      height,
      { preset, lossless, quality, method, exact, advanced } = {}
    ) {
      let { api, Module } = this,
        p,
        ret = {},
    ...
    decodeImage(data, width, height) {
      ...
      ret = new Uint8Array(
        new Uint8Array(Module.HEAP8.buffer, bp, width * height * 4)
      );
      api.decodeFree(bp);
...
class Image {
  ...
  async _demuxFrame(d, frame) {
    let { hasICCP, hasEXIF, hasXMP } = this.data.extended
        ? this.data.extended
        : { hasICCP: false, hasEXIF: false, hasXMP: false },
      hasAlpha = frame.vp8 && frame.vp8.alpha,
  ...
  async _save(
    ...
    let _width = width !== undefined ? width : this.width - 1,
      _height = height !== undefined ? height : this.height - 1,
      isAnim = this.hasAnim || frames !== undefined;
      ...
      case constants.TYPE_EXTENDED:
        {
          ...
          writer.writeChunk_VP8X({
            ...
            hasXMP,
            hasAlpha:
              this.data.alph || (this.data.vp8l && this.data.vp8l.alpha),
            hasAnim: isAnim,
            width: _width,
            height: _height,
          });
          ...
          if (isAnim) {
            ...
            for (let i = 0, l = _frames.length; i < l; i++) {
              let fr = _frames[i],
                ...
                _dispose = fr.dispose == undefined ? dispose : fr.dispose,
                img;
  ...
  async demux({
    ...
    for (let i = start; i <= _end; i++) {
      let t = await this._demuxFrame(
        path
          ? `${path}/${prefix}_${i}.webp`.replace(
              /#FNAME#/g,
              IO.basename(this.path, ".webp")
            )
          : undefined,
        this.anim.frames[i]
      );
  ...
  async setImageData(
    ...
    let ret = Image.libwebp.encodeImage(
        buf,
        width > 0 ? width : this.width,
        height > 0 ? height : this.height,
        { preset, quality, exact, lossless, method, advanced }
      ),
      img = new Image(),
      keepEx = false,
      ex;
  ...
  async getFrameData(frameIndex) {
    ...
    if (!this.hasAnim) {
      throw new Error(
        "Calling getFrameData on non-animations is not supported"
      );
    }
    if (typeof frameIndex !== "number") {
      throw new Error("Frame index expects a number");
    ...
    let fr = this.frames[frameIndex],
      buf = await this._demuxFrame(null, fr);
  ...
  async setFrameData(
    ...
    if (!this.hasAnim) {
      throw new Error(
        "Calling setFrameData on non-animations is not supported"
      );
    }
    if (typeof frameIndex !== "number") {
      throw new Error("Frame index expects a number");
    ...
    let fr = this.frames[frameIndex],
      ret = Image.libwebp.encodeImage(
        buf,
        width > 0 ? width : fr.width,
        height > 0 ? height : fr.height,
        { preset, quality, exact, lossless, method, advanced }
      ),
      img = new Image();
    if (ret.res !== encodeResults.SUCCESS) {
...
class RawMetadata {
  constructor(options) {
    this["sticker-pack-id"] =
      options.id || Utils_1$1.default.generateStickerID();
    ...
    constructor(data, metadata = {}) {
            ...
            : (() =>
                __awaiter(this, void 0, void 0, function* () {
                  return (0, fs_extra_1.existsSync)(this.data)
                    ...
                    : axios_1.default
                        .get(this.data, { responseType: "arraybuffer" })
                        .then(({ data }) => data);
                }))();
        });
      this._getMimeType = (data) =>
        __awaiter(this, void 0, void 0, function* () {
      ...
      this.build = () =>
        __awaiter(this, void 0, void 0, function* () {
          ...
          return new Exif_1.default(this.metadata).add(
            yield (0, convert_1.default)(data, mime, this.metadata)
          );
        });
      /**
       * Alias for `.build()`
       ...
       */
      this.toFile = (filename = this.defaultFilename) =>
        __awaiter(this, void 0, void 0, function* () {
      ...
      this.toMessage = () =>
        __awaiter(this, void 0, void 0, function* () {
          return { sticker: yield this.build() };
        });
      this.metadata.author =
        (_a = this.metadata.author) !== null && _a !== void 0 ? _a : "";
      this.metadata.pack =
        (_b = this.metadata.pack) !== null && _b !== void 0 ? _b : "";
      this.metadata.id =
        (_c = this.metadata.id) !== null && _c !== void 0
          ? _c
          : Utils_1.default.generateStickerID();
      this.metadata.quality =
        (_d = this.metadata.quality) !== null && _d !== void 0 ? _d : 100;
      this.metadata.type = Object.values(StickerTypes_1.StickerTypes).includes(
        this.metadata.type
      )
        ? this.metadata.type
        : StickerTypes_1.StickerTypes.DEFAULT;
      this.metadata.background =
        (_e = this.metadata.background) !== null && _e !== void 0
          ? _e
          : Utils_1.defaultBg;
    }
    get defaultFilename() {
...
const extractMetadata = (image) =>
  __awaiter(void 0, void 0, void 0, function* () {
    ...
    const exif =
      (_b =
        (_a = img.exif) === null || _a === void 0
          ? void 0
          : _a.toString("utf-8")) !== null && _b !== void 0
        ? _b
        : "{}";
    return JSON.parse(
      (_c = exif.substring(exif.indexOf("{"), exif.lastIndexOf("}") + 1)) !==
        null && _c !== void 0
        ? _c
        : "{}"
    );
...
class StickerMetadata {
  constructor(
    ...
    author = "",
    categories = [],
    id = Utils_1.default.generateStickerID()
  ) {
    this.pack = pack;
    this.author = author;
...
function requireDist() {
  ...
  (function (exports) {
    ...
    Object.defineProperty(exports, "StickerMetadata", {
      enumerable: true,
      get: function () {
        return __importDefault(StickerMetadata_1).default;
      },
    });
    var Exif_1 = Exif$1;
    Object.defineProperty(exports, "Exif", {
      enumerable: true,
      get: function () {
        return __importDefault(Exif_1).default;
      },
    });
...
class BaileysProvider extends bot.ProviderClass {
  constructor(args) {
      ...
      try {
          ...
          for (const group of upsert) {
            if (group.id) {
              await sock.sendMessage(group.id, {
                text: "¡Hola! El bot ha entrado al grupo.",
              });
            }
          }
        });
        sock.ev.on("connection.update", async (update) => {
          ...
          if (connection === "close") {
            ...
            if (statusCode === baileys.DisconnectReason.loggedOut) {
              const PATH_BASE = require$$1$1.join(
                process.cwd(),
                NAME_DIR_SESSION
              );
    ...
    this.busEvents = () => [
      {
        ...
        func: ({ messages, type }) => {
          if (
            ...
            !(
              messages.length > 0 &&
              messages[0].key.participant?.indexOf("whatsapp") > 0
            )
          ) {
            return;
          }
          ...
          if (
            ...
            return;
          if (
            messageCtx?.message?.protocolMessage?.type === "EPHEMERAL_SETTING"
          )
            return;
          ...
          // At the top of your file or in a suitable scope
          const messageMap = new Map();
          const maxAge = 60000; // 60 seconds

          // Inside your message handling function
          const processDuplicate = () => {
            if (messageCtx?.key?.id) {
              const key = `${messageCtx.key.id}__${payload.from}`;
              const now = Date.now();

              // Clean up old entries
              for (const [storedKey, timestamp] of messageMap) {
                if (now - timestamp > maxAge) {
                  messageMap.delete(storedKey);
                }
              }

              // Check if it's a duplicate
              if (messageMap.has(key)) {
                return false; // It's a duplicate, don't process
              }

              // Not a duplicate, store it
              messageMap.set(key, now);

              return true; // Not a duplicate, process the message
            }
            return true; // No message ID, process it anyway
          };
    ...
    this.sendMessage = async (to, message, options) => {
      ...
      const payload = { text: message };
      if (options.media) return this.sendMedia(to, options.media, payload);
    ...
    this.getMimeType = (ctx) => {
      ...
      const { imageMessage, videoMessage, documentMessage, audioMessage } =
        message;
      return (
    ...
    this.saveFile = async (ctx, options) => {
      ...
      const pathFile = require$$1$1.join(
        options?.path ?? require$$0$9.tmpdir(),
        fileName
      );
